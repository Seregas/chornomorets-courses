...
> - **Зʼявився Google-вхід** (GoogleSignIn SDK) — і на ньому тепер тримається все особисте:
>   підписки, оплати, домашки, питання. Ключ — `sub` із токена, пошта поруч. Анонімний
>   `deviceId` лишився тільки для логів із телефона: їх пишуть ще до входу й саме тоді,
>   коли ламається вхід.
