...
- **Sign in with Apple** — за guideline 4.8 обовʼязковий: Google-вхід створює
  основний акаунт застосунку, а жоден із винятків (власна система входу,
  корпоративні акаунти, державний eID, клієнт конкретного сервісу) сюди не
  підходить. Для TestFlight це не блокер, для публікації — блокер. Вхід через
  Apple має заводити той самий `accounts`-запис; Google лишається окремо як
  доступ до відео на Drive. Видалення акаунта (5.1.1(v)) уже є.
